...
function(DCMTK_SETUP_ANDROID_EMULATOR)
    ...
    if(NOT ANDROID_EMULATOR_PROGRAM OR NOT ANDROID_ANDROID_PROGRAM OR NOT ANDROID_ADB_PROGRAM)
        ...
        message("Trying to find AVD in ${ANDROID_SDK_ROOT} calling ${ANDROID_ANDROID_PROGRAM}")
        ...
        message("Available AVDs: ${ANDROID_AVAILABLE_AVDS}")
        message("AVD: ${ANDROID_EMULATOR_AVD}")
    endif()
endfunction()
...
function(DCMTK_ANDROID_LIST_EMULATORS ONLINE OFFLINE)
    ...
    message("ADB: ${ANDROID_ADB_PROGRAM}")
...
function(DCMTK_ANDROID_START_EMULATOR VAR)
    DCMTK_SETUP_ANDROID_EMULATOR()
    message("DCMTK_SETUP_ANDROID_EMULATOR called, ANDROID_EMULATOR_AVD = ${ANDROID_EMULATOR_AVD}")
    if(NOT ANDROID_EMULATOR_AVD)
        ...
        if(CMAKE_HOST_SYSTEM MATCHES "Windows.*")
            set(COMMAND sh -c "${ANDROID_EMULATOR_PROGRAM} -avd ${ANDROID_EMULATOR_AVD} -no-boot-anim -prop qemu.uuid=${EMULATOR_UUID} >${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/android-emulator.log 2>&1 < /dev/null &")
        else()
            set(COMMAND sh -c "${ANDROID_EMULATOR_PROGRAM} -avd ${ANDROID_EMULATOR_AVD} -no-window -no-boot-anim -prop qemu.uuid=${EMULATOR_UUID} >${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/android-emulator.log 2>&1 < /dev/null &")
        endif()
        message(${COMMAND})
        execute_process(
            COMMAND ${COMMAND}
